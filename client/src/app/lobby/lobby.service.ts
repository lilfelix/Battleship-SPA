...
export class LobbyService implements OnInit {
  ...
  constructor(private http: HttpService, private authService: AuthService, private router: Router, private wsService: WebsocketService) {
  ...
  ngOnInit() {
    this.wsService.lobbyEventSource
    .subscribe((event: any) => {
  ...
  processLobbyEvent(event: any) {
    switch (event.status) {
      ...
      case 'NEW_USER':
      this.displayUserSource.next(event.payload);
      break;
      case 'CHALLENGE':
      this.processChallenge(event.payload);
      break;
      default:
  ...
  // New challenge event received on websocket.
